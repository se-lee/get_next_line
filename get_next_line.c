...
/*   Updated: 2021/04/07 13:31:35 by selee            ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int get_next_line(int fd, char **line)
{
	...
	{
		size_read = 0;
		while (size_read < BUFFER_SIZE) //fill buf with zero;
		{
			buf[size_read] = 0;
			size_read++;
		}
		size_read = read(fd, buf, BUFFER_SIZE); //read from fd and store into buf
		buf[size_read] = '\0';
		if (size_read > 0)
		{
